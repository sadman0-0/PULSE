...
# The website's pages are opened straight from disk (file://) or from a
# simple static file server, which is a different "origin" than this API,
# so the browser will block the request unless CORS is explicitly allowed.
CORS(app)
...
def health_check():
    return jsonify({"status": "ok", "message": "Habit Harvest tracking server is running"})
